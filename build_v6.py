...
def build():
    ...
    # 4. TRIPLE FLATTEN Torch DLLs
    # Locations:
    # 1. dist/SassyCam/ (App Root)
    # 2. dist/SassyCam/_internal/ (Internal Root)
    # 3. dist/SassyCam/_internal/torch/ (Package Root - Next to _C.pyd)
    ...
    print("Triple Flattening Torch DLLs...")
    if os.path.exists(torch_lib):
        for file in os.listdir(torch_lib):
            if file.endswith(".dll"):
                ...
                print(f" -> Distributed {file} to all 3 locations")
    ...
    with open(init_path, 'w', encoding='utf-8') as f:
        for line in lines:
            ...
            # Disable the definition (Optional, but cleaner if we are bypassing)
            # Actually, disabling the call is enough.
            else:
                f.write(line)

    print("Build v6 Complete.")
