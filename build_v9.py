...
def build():
    ...
    # 5. Copy Python DLLs to Torch Folder (New in v9)
    # torch_python.dll might depend on python314.dll being reachable
    python_dlls = glob.glob(os.path.join(internal_dest, "python*.dll"))
    for dll in python_dlls:
        shutil.copy2(dll, os.path.join(package_dest, os.path.basename(dll)))
        print(f" -> Copied {os.path.basename(dll)} to torch package")

    # 6. HUNT AND COPY TBB12.DLL
    ...
    for cand in candidates:
        if os.path.exists(cand):
            ...
            shutil.copy2(cand, os.path.join(package_dest, "tbb12.dll"))
            tbb_found = True
            break
